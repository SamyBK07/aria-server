...
app.post('/subscribe', (req, res) => {
  ...
  db.prepare('INSERT INTO subscriptions (id, subscription) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET subscription = excluded.subscription')
...
cron.schedule('* * * * *', async () => {
  ...
  const hhmm5 =
    String(d5.getHours()).padStart(2, '0') + ':' +
    String(d5.getMinutes()).padStart(2, '0');
  ...
  for (const r of reminders) {
    ...
    const alreadyDone = db.prepare(
      'SELECT 1 FROM reminders WHERE id = ? AND last_triggered = ?'
    ).get(r.id, triggerKey);

    if (alreadyDone) continue;

    db.prepare('UPDATE reminders SET last_triggered = ? WHERE id = ?')
      .run(triggerKey, r.id);

    const sub = db.prepare('SELECT subscription FROM subscriptions WHERE id = ?')
      .get(r.user_id);
